...
public class Soldier {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        for (Message msg : messages) {
            ...
            if (((decoded >> 12) & 0x3) == Constants.MSG_SYMMETRY)
                RobotPlayer.symmetryMode = (decoded >> 14) & 0x3;
        }
        ...
        if (enemies.length > 0) {
            ...
            if (tower != null) {
                ...
                if (rc.canSendMessage(tower.getLocation(), symMsg))
                    rc.sendMessage(tower.getLocation(), symMsg);
                if (rc.canSendMessage(tower.getLocation(), enemyMsg))
                    rc.sendMessage(tower.getLocation(), enemyMsg);
            }
        }
        ...
        if (targetTower != null) {
            ...
            if (rc.isMovementReady()) {
                Utils.moveGreedy(rc, targetTower.getLocation());
                return;
            }
        }
        ...
        for (MapLocation ruin : ruins) {
            if (rc.senseMapInfo(ruin).getMark() == PaintType.EMPTY)
                continue;
            if (rc.getLocation().distanceSquaredTo(ruin) > 9) {
                if (rc.isMovementReady())
                    Utils.moveGreedy(rc, ruin);
                break;
            }
            ...
            for (MapInfo tile : tiles) {
                if (tile.getMark() == PaintType.EMPTY || tile.getPaint() == tile.getMark())
                    continue;
                if (!rc.canAttack(tile.getMapLocation()) || !rc.isActionReady())
                    continue;
        ...
        if (rc.isActionReady()) {
            ...
            if (cur.getPaint() == PaintType.EMPTY && rc.canAttack(rc.getLocation()))
