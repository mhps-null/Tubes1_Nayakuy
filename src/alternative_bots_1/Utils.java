...
public class Utils {
    ...
    public static RobotInfo nearestAllyPaintTower(RobotController rc) throws GameActionException {
        ...
        int bestDist = Integer.MAX_VALUE;
        for (RobotInfo r : allies) {
            if (r.getType() == UnitType.LEVEL_ONE_PAINT_TOWER || r.getType() == UnitType.LEVEL_TWO_PAINT_TOWER
    ...
    public static boolean moveGreedy(RobotController rc, Direction targetDir) throws GameActionException {
        if (!rc.isMovementReady() || targetDir == Direction.CENTER)
            return false;
    ...
    public static MapLocation getEnemyEstimate(RobotController rc, int symmetryMode, MapLocation towerLoc)
    ...
    public static int hitungSimetriDariObservasi(RobotController rc, MapLocation enemyPos, MapLocation towerLoc)
            throws GameActionException {
        int W = rc.getMapWidth(), H = rc.getMapHeight();
    ...
    public static MapLocation tileToward(RobotController rc, MapLocation target, UnitType robotType)
            throws GameActionException {
        if (rc.getLocation().equals(target))
            return null;
        Direction best = rc.getLocation().directionTo(target);
        Direction[] candidates = { best, best.rotateLeft(), best.rotateRight(), best.rotateLeft().rotateLeft(),
                best.rotateRight().rotateRight(), best.opposite().rotateLeft(), best.opposite().rotateRight(),
                best.opposite() };
    ...
    public static UnitType inferTowerType(RobotController rc, MapLocation ruin) throws GameActionException {
        UnitType[] priority = { UnitType.LEVEL_ONE_PAINT_TOWER, UnitType.LEVEL_ONE_MONEY_TOWER,
                UnitType.LEVEL_ONE_DEFENSE_TOWER };
        for (UnitType t : priority)
    ...
    public static RobotInfo weakestEnemyTower(RobotInfo[] enemies) {
        ...
        for (RobotInfo e : enemies) {
            if (!e.getType().isTowerType())
                continue;
            if (best == null || e.getHealth() < best.getHealth())
                best = e;
        }
        return best;
    }
