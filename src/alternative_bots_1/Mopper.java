...
public class Mopper {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        if (combatEnemies.length > 0 && rc.isActionReady()) {
            ...
            for (Direction d : new Direction[] { Direction.NORTH, Direction.EAST, Direction.SOUTH, Direction.WEST }) {
                ...
                for (RobotInfo e : combatEnemies) {
                    if (e.getLocation().distanceSquaredTo(s1) <= 2 || e.getLocation().distanceSquaredTo(s2) <= 2)
                        count++;
                }
                if (count > bestCount) {
                    bestCount = count;
            ...
            for (RobotInfo e : combatEnemies) {
                if (e.getPaintAmount() < target.getPaintAmount())
                    target = e;
            }
        ...
        MapLocation enemyPaintTile = null;
        ...
        for (MapInfo tile : nearbyTiles) {
            ...
            if (d < bestDist) {
                bestDist = d;
                enemyPaintTile = tile.getMapLocation();
            }
        }
        if (enemyPaintTile != null) {
            if (rc.isMovementReady() && bestDist > 2)
                Utils.moveGreedy(rc, enemyPaintTile);
        ...
        for (MapLocation ruin : ruins) {
            ...
            if (done != null)
                rc.completeTowerPattern(done, ruin);
            return;
        }
        ...
        if (nearestSoldier != null && rc.isMovementReady()) {
            ...
            if (rc.canSenseLocation(enemyTarget)) {
                ...
                if (botAtTarget == null || !botAtTarget.getType().isTowerType()
                        || botAtTarget.getTeam() == rc.getTeam()) {
